---
title: "DEG Markers for Microglia Subclusters in 10 samples"
author: "Saumya"
date: "`r Sys.Date()`"
output:
  html_document:
...
# `r colorize('UMAP of Microglia Subcluster before merging and filtering', 'blue')` 

```{r,results='asis', echo=FALSE, message=FALSE, warning=FALSE}
data.combined <- readRDS("Cluster2filt_data_Microglia_6_subclusters.RDS")
DimPlot(data.combined, reduction = "umap", label = TRUE, repel = TRUE, raster=FALSE)+ ggtitle('Microglia Subcluster')
```

<!-- # ```{r, echo=FALSE, message=FALSE, warning=FALSE} -->
<!-- # # Subset 5 subclusters to filter Oligo Microglia -->
<!-- # # Load data -->
<!-- # data.combined <- readRDS("Cluster2filt_data_Microglia_5_subclusters.RDS") -->
<!-- #  -->
<!-- # # Subset the data to exclude "Oligo Microglia" -->
<!-- # data.combined.subset <- subset(data.combined, subset = name_5_clusters != "Oligo Microglia") -->
<!-- # ``` -->
<!-- #  -->
<!-- # ```{r, echo=FALSE, message=FALSE, warning=FALSE} -->
<!-- # # Save the subsetted data as a new RDS file -->
<!-- # saveRDS(data.combined.subset, file = "Cluster2filt_data_Microglia_4_subclusters.RDS") -->
<!-- # ``` -->


# `r colorize('UMAP of Microglia Subcluster after merging and filtering', 'blue')` 

```{r,results='asis', echo=FALSE, message=FALSE, warning=FALSE}
data.combined.subset <- readRDS("Cluster2filt_data_Microglia_4_subclusters.RDS")
DimPlot(data.combined.subset, reduction = "umap", label = TRUE, repel = TRUE, raster=FALSE)+ ggtitle('Microglia Merged Subcluster After Oligo Microglia Filtering')
```
...
```{r, results= FALSE, echo=FALSE, message=FALSE, warning=FALSE}
pdf(paste("pdf_microglia_merged_4_clusters_umap.pdf"), width=10, height=8, useDingbats = FALSE)
DimPlot(data.combined.subset, reduction = "umap")+ ggtitle('Microglia Merged and Filterd Subcluster')
dev.off()
```


# `r colorize('Identifying markers in Microglia Cluster', 'blue')` {.tabset}

<!-- # ```{r, eval = FALSE, echo=FALSE, message=FALSE, warning=FALSE} -->
<!-- # data.combined <- readRDS("Cluster2filt_data_Microglia_4_subclusters.RDS") -->
...
<!-- #  -->
<!-- # # Save markers -->
<!-- # write.csv(data.markers, file='data_volcano_deg_group_markers_microglia_4_subclusters.csv') -->
<!-- # ``` -->
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# Read Markers .csv
data.markers = read.csv('data_group_markers_microglia_4_subclusters.csv')
...
positive.markers = read.csv('data_group_markers_microglia_4_subclusters.csv')
...
write.csv(positive.markers, file='data_positive_group_markers_microglia_4_subclusters.csv')
...
# Positive markers for Microglia Cluster {.tabset}
...
# Negative markers for Microglia Cluster {.tabset}
...
# All markers for Microglia Cluster {.tabset}
...
# `r colorize('Dot Plots of Top 30 markers for Microglia Subcluster', 'blue')` {.tabset}

```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Dot plot for microglia cluster
...
for (cond in unique(top_markers$cluster)) {
  ...
  dot_plot <- DotPlot(object = data.combined,
              ...
              labs(title = paste("Microglia Cluster Markers",cond)) +
...
#load subset data
data.combined <- readRDS("Cluster2filt_data_Microglia_4_subclusters.RDS")
Idents(data.combined) <- "name_5_clusters"

# Read positive marker data
data.markers <- read.csv("data_cluster_markers_microglia_4_subclusters.csv")
...
for (cond in unique(top_markers$cluster)) {
  ...
  # Store the dot plot in the list
  cluster_dot_plots[[paste(cond)]] <- dot_plot
}

```

## Group Markers {.tabset}
```{r,results='asis', echo=FALSE, message=FALSE, warning=FALSE}
# Print all dot plots
cat('###',"CONTROL",' \n')
...
# Print all dot plots
cat('###',"T cells",' \n')
print(cluster_dot_plots["T cells"])
cat('\n \n')
cat('###',"Microglia 3",' \n')
print(cluster_dot_plots["Microglia 3"])
cat('\n \n')
cat('###',"Microglia 2",' \n')
print(cluster_dot_plots["Microglia 2"])
cat('\n \n')
cat('###',"Microglia 1",' \n')
print(cluster_dot_plots["Microglia 1"])
cat('\n \n')
```


# `r colorize('Volcano plot for DEGs for Microglia Cluster', 'blue')` {.tabset}

```{r,results='asis', echo=FALSE, message=FALSE, warning=FALSE}

# Read the DEGs from the CSV files
de <- read.csv('data_volcano_deg_group_markers_microglia_4_subclusters.csv')
# List of oligo types
types <- c("CR")
...
volcanoplot <- function() {
  ...
  coord_cartesian(ylim = c(0, 100), xlim = c(-2, 2)) + # set limits
  ...
  ggtitle(paste0('DEGs in Microglia Subcluster CR vs CONTROL')) + # Plot title 
  theme_bw() +
...
```{r, results= FALSE, echo=FALSE, message=FALSE, warning=FALSE}
pdf(paste("pdf_microglia_deg_volcano_plots.pdf"), width=10, height=8, useDingbats = FALSE)
print(p)
dev.off()
```

```{r,results= FALSE, echo=FALSE, message=FALSE, warning=FALSE}
pdf(paste("pdf_group_microglia_dot_plots.pdf"), width=10, height=8, useDingbats = FALSE)
print(dot_plots)
dev.off()
```

```{r, results= FALSE, echo=FALSE, message=FALSE, warning=FALSE}
pdf(paste("pdf_cluster_microglia_dot_plots.pdf"), width=10, height=8, useDingbats = FALSE)
print(cluster_dot_plots)
dev.off()
```
